...
export const createUser = asyncHandler(async (req, res) => {
  ...
  // Validate that the department actually exists (and is active) before
  // attaching a user to it.
  const existingDepartmentForUser = await Department.findOne({
    _id: department,
    status: "active",
  });

  if (!existingDepartmentForUser) {
    throw new ApiError(404, "Department not found");
  }
...
export const createAsset = asyncHandler(async (req, res) => {
  ...
  // --- FIX ---
  // Previously the department id from req.body was assigned straight onto
  // the asset with no check that it actually referred to a real,
  // active Department document. That let assets get created (or restored)
  // pointing at a department that doesn't exist. Now we look it up first
  // and reject the request instead of silently letting a bad id through.
  const existingDepartment = await Department.findOne({
    _id: department,
    status: "active",
  });
  ...
  if (existingAsset) {
    ...
    existingAsset.isShared = isShared;
    existingAsset.count = count;
    existingAsset.department = existingDepartment._id;
    existingAsset.createdBy = req.user._id;
    existingAsset.isDeleted = false;
...
export const updateAsset = asyncHandler(async (req, res) => {
  ...
  // --- FIX ---
  // Same issue as createAsset: only touch asset.department after confirming
  // the new department id actually exists and is active.
  if (department !== undefined) {
    ...
    asset.department = existingDepartment._id;
  }
