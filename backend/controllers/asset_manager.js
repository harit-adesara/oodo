...
export const registerAsset = asyncHandler(async (req, res) => {
  ...
  if (!name || !assetTag || count === undefined || !department) {
    throw new ApiError(400, "Name, assetTag, count, and department are required fields");
  }

  const numericCount = Number(count);
  if (isNaN(numericCount) || numericCount < 0 || !Number.isInteger(numericCount)) {
...
export const deleteAsset = asyncHandler(async (req, res) => {
  ...
  const activeAllocations = await Allocation.findOne({ asset: assetId, status: "approved" });
