...
import { deptHeadGetAllocations } from "../../api.js";
...
const DeptAllocations = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await deptHeadGetAllocations();
        setAllocations(res.data?.data || []);
      } catch (err) {
        setError(err.response?.data?.msg || "Failed to load allocations.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className={page}>
      ...
      <p className="text-[#6b6b7a] text-sm mt-1.5">
        Asset allocation requests from employees in your department
      </p>
      ...
      <div className={`${card} mt-8`}>
        ...
        {loading ? (
          ...
          <p className="text-[#6b6b7a] text-sm">No allocation requests yet.</p>
        ) : (
            ...
            <table className={table}>
              <thead>
                <tr>
                  ...
                  <th className={th}>Allocated by</th>
              ...
              <tbody>
                {allocations.map((req) => (
                  <tr key={req._id}>
                    ...
                    <td className={td}>{req.allocatedBy?.name || "—"}</td>
                    <td className={td}>
